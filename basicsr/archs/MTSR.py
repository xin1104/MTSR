...
class HAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask=None):
        ...
        # 残差 + DropPath
        x = shortcut * self.skip_scale + self.drop_path(attn_x) + conv_x * self.conv_scale

        # FFN
        x_norm = self.norm2(x)
        ffn_out = self.ffn(x_norm, x_size)  #  [B, L, C]

        x = x * self.skip_scale2 + self.drop_path(ffn_out)
        return x
...
class MTSR(nn.Module):
    def __init__(self,
        ...
        self.shallow_refine = ShallowRefine(embed_dim)
        ...
        if resi_connection == '1conv':
            ...
            #     nn.Conv2d(embed_dim, embed_dim // 4, 1, 1, 0, bias=False),
            #     nn.GELU(),
            #     nn.Conv2d(embed_dim // 4, embed_dim // 4, 3, 1, 1, groups=embed_dim // 4, bias=False),
            #     nn.GELU(),
            #     nn.Conv2d(embed_dim // 4, embed_dim, 1, 1, 0, bias=False),
            # )
        ...
        # elif self.upsampler == 'pixelshuffledirect':
        #     self.upsample = UpsampleOneStepUseDS(upscale, embed_dim, num_out_ch)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for lightweight SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x)) + self.shallow_refine(x) + x
            x = self.upsample(x)
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch):
        ...
        m = []
        m.append(nn.Conv2d(num_feat, (scale**2) * num_out_ch, 1, 1, 0))
...
class ShallowRefine(nn.Module):
    def __init__(self, channels: int):
        ...
        self.dsconv3 = DepthwiseSeparableConv(channels, channels, 3, padding=1)
        self.dsconv5 = DepthwiseSeparableConv(channels, channels, 5, padding=2)
        self.dsconv7 = DepthwiseSeparableConv(channels, channels, 7, padding=3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        f3 = self.dsconv3(fl)
        f5 = self.dsconv5(f3 * self.w1 + fl)
        f7 = self.dsconv7(f5 * self.w2 + fl)

        out = self.ln((f3 + f5 + f7).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
